...
# ERM using deterministic

# ==========================================
# 0. OUTPUT PATHS
# ==========================================
RESULTS_DIR = os.path.join('results', 'test7')
...
MI_MC_SAMPLES = 10       # MC samples for MI/KL mixture estimation (proposed)
SEED = 2
...
# ==========================================
def evaluate_inference(model, loader, repeats=1, weight_mc_samples=1, scenario_name=None):
    model.eval()
            ...
            for batch_x, batch_y in loader:
                ...
                if scenario_name == 'proposed':
                    for _ in range(weight_mc_samples):
                        ...
                        total_mc += 1
                elif scenario_name == 'erm':
                    theta_mean = model.mu if scenario_name == 'erm' else None
...
if __name__ == "__main__":
    ...
    if not RUN_SWEEP:
        ...
        for seed in SEEDS:
            ...
            model_erm = train_scenario('erm', train_loader, n_trains, mode='train', objective='bound', seed=seed, use_cache=False)
            loss_erm, acc_erm = evaluate_inference(
                model_erm,
                test_loader,
                repeats=EVAL_REPEATS,
            ...
            # Proposed: heuristic objective sweep
            for reg_coeff in REG_COEFF_GRID:
                for reg_alpha in REG_ALPHA_GRID:
                    for reg_beta in REG_BETA_GRID:
                        model_prop = train_scenario(
