...
# 0. OUTPUT PATHS
# ==========================================
RESULTS_DIR = os.path.join('results', 'test12_l2')
...
if __name__ == "__main__":
    ...
    if not RUN_SWEEP:
        ...
        BETA_COEFF_GRID = [0.1]
        GAMMA_COEFF_GRID = [0.5, 0.1, 0.05, 0.01, 0.005, 0.001, 0.0005, 0.0001]
        EPOCHS_GRID = [150]
        use_cache = False
        verbose = False
                                ...
                                for lr_decay_step in LR_DECAY_STEP_GRID:
                                    for lr_decay_gamma in LR_DECAY_GAMMA_GRID:
                                        ...
                                        for gamma_coeff in GAMMA_COEFF_GRID:
                                            tasks.append({
                                                "scenario": "l2", "seed": seed, "hidden_dim": hidden_dim, "batch_size": BATCH_SIZE, "moon_noise": MOONS_NOISE, "n_samples": N_SAMPLES, "epochs": epochs,
                                                "m_artificial_channels": m_artificial_channels, "lr": lr,
                                                "lr_decay_step": lr_decay_step, "lr_decay_gamma": lr_decay_gamma, "gamma_coeff": gamma_coeff,
                                                "use_cache": use_cache, "verbose": verbose,
                                            })
