...
# ERM using deterministic, BEC, given U and S

# ==========================================
# 0. OUTPUT PATHS
# ==========================================
RESULTS_DIR = os.path.join('results', 'test15')
...
def compute_component_expected_kl(model):
    # compute the KL divergence D(P_{W|US} || Q_W)
    mu = model.mu
    ...
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in loader:
            ...
            if model.is_bnn:
                ...
                component_expected_kl = torch.tensor(0.0, device=ce_loss.device)
