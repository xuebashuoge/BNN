...
# 0. OUTPUT PATHS
# ==========================================
RESULTS_DIR = os.path.join('results', 'test3')
...
REG_ALPHA = 0.05        # Weight for MI term in the proposed bound
REG_BETA = 0.5         # Weight for KL(P_{W|w,S} || Q) term in the proposed bound
SEED = 2

# Sweep configuration
SEEDS = [1, 2, 3, 4, 5]
REG_COEFF_GRID = [0.0, 0.01, 0.05, 0.1]
REG_ALPHA_GRID = [0.0, 0.01, 0.05, 0.1]
REG_BETA_GRID = [0.01, 0.1, 0.5, 1.0]
...
MU_M_TR, STD_M_TR = 1.0, 1.0  # Almost no fading variation
...
MU_M_TE, STD_M_TE = 1.0, 1.0   # Severe fading
...
def get_dataloaders(n_samples, batch_size=32, seed=0):
...
# ==========================================
def train_scenario(scenario_name, loader, n_samples, mode='perfect', objective='bound', reg_coeff=None, reg_alpha=None, reg_beta=None, use_cache=True):
...
if __name__ == "__main__":
    ...
    if not RUN_SWEEP:
        ...
        model_erm_perfect = train_scenario('erm', train_loader, n_trains, mode='perfect', objective='bound', use_cache=True)
        ...
        model_erm = train_scenario('erm', train_loader, n_trains, mode='train', objective='bound', use_cache=True)
        ...
        model_prop = train_scenario('proposed', train_loader, n_trains, mode='train', objective='bound', use_cache=True)
        ...
        model_prop_perfect = train_scenario('proposed', train_loader, n_trains, mode='perfect', objective='bound', use_cache=True)
        ...
        for seed in SEEDS:
            set_seed(seed)
            train_loader, test_loader, n_trains = get_dataloaders(N_SAMPLES, batch_size=BATCH_SIZE, seed=seed)

            # Baseline ERM (train channel) per seed
            model_erm = train_scenario('erm', train_loader, n_trains, mode='train', objective='bound', use_cache=False)
            loss_erm, acc_erm = evaluate_inference(
                model_erm,
                test_loader,
                repeats=EVAL_REPEATS,
            ...
            # Proposed: bound objective sweep (reg_coeff only)
            for reg_coeff in REG_COEFF_GRID:
